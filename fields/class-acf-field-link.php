...
if( ! class_exists('acf_field_link_extension') ) :
    ...
    class acf_field_link_extension {
        ...
        /**
         * When saving link, store post_id and boolean to know if the link title is the post title
         * @param $value
         * @param int $post_id
         * @param array $field
         * @return mixed
        ...
        public function loadValue($value, $post_id=0, $field=array()){
            ...
            if( $field['type'] == 'link' && isset($value['post_id']) && $value['post_id']){
                ...
                $base_url = strtok(strtok($value['url'],'?'),'#');

                if( $url != $base_url) {

                    $value['url'] = str_replace($base_url, $url, $value['url']);
                    $update = true;
                }
